...
class MidtransHelper {
    ...
    const WK_MIDTRANS_SANDBOX_URL = 'https://app.sandbox.midtrans.com/snap';
    const WK_MIDTRANS_LIVE_URL = 'https://app.midtrans.com/snap';
    const WK_MIDTRANS_ACCESS_TOKEN_URI = 'v1/transactions';
    const WK_MIDTRANS_COMMERCE_ATTRIBUTION_ID = 'QloApp_SI';

	public function __construct() {
		$this->_http = new HttpHelper;
        $this->_apiUrl = self::isSanboxEnvironment() ? self::WK_MIDTRANS_SANDBOX_URL : self::WK_MIDTRANS_LIVE_URL;
		$this->_setAPIVersion('1');
	}

	protected function _setDefaultHeaders() {
        $this->_http->addHeader("Midtrans-Partner-Attribution-Id: " . self::WK_MIDTRANS_COMMERCE_ATTRIBUTION_ID);
    ...
    protected function _getToken() {
        ...
        $this->_http->setAuthentication(Configuration::get('WK_MIDTRANS_COMMERCE_CLIENT_ID') . ":" . Configuration::get('WK_MIDTRANS_COMMERCE_CLIENT_SECRET'));
    ...
    {
        if (Configuration::get('WK_MIDTRANS_COMMERCE_PAYMENT_MODE') == 'sandbox') {
            return true;
        }
        return false;
    }
    ...
    {
        if (Configuration::get('WK_MIDTRANS_COMMERCE_PAYMENT_MODE') == 'sandbox') {
            return self::WK_MIDTRANS_SANDBOX_URL.$uri;
        }

        return self::WK_MIDTRANS_LIVE_URL.$uri;
    }
