...
include_once(dirname(__FILE__) . '/Helpers/MidtransHelper.php');

class MidtransOrders extends MidtransHelper
{
    private function _createOrder($postData) {
        $this->_checkToken();
        $this->_http->resetHelper();
        $this->_setDefaultHeaders();
        // $this->_http->addHeader("Midtrans-Request-Id: " . $this->getUUID());
    ...
    private function _refundCapture($transactionID, $assertion, $postData) {
        ...
        $this->_setDefaultHeaders();
        $this->_http->addHeader("Midtrans-Request-Id: " . $this->getUUID());
        $this->_http->addHeader("Midtrans-Auth-Assertion: " . $assertion);
        $this->_setAPIVersion('2');
