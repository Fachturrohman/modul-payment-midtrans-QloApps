...
class WkMidtransCommerceWebhook
{
    public function capturePending($eventData)
    {
        $transaction_id = $eventData['resource']['id'];

        $objMidtransOrder = new WKMidtransCommerceOrder();
        if ($objMidtransOrder->getTransactionDetailsByMidtransTransaction(
            $transaction_id
        )) {
            $ppOrderStatus = $eventData['resource']['status'];
            $this->updateOrderCaptureMidtransOrderStatus($transaction_id, $ppOrderStatus);
        }
    }
    ...
    public function captureRefunded($eventData)
    {
        ...
        $id = Db::getInstance()->getValue(
            'SELECT `id_midtrans_commerce_refund` FROM `'._DB_PREFIX_.'wk_midtrans_commerce_refund`
            WHERE `midtrans_refund_id` =  "' . pSQL($refundID) .'"'
        );

        $refundObj = new WkMidtransCommerceRefund($id);
    ...
    public function captureDenied($eventData)
    {
        $transaction_id = $eventData['resource']['id'];
        $objMidtransOrder = new WKMidtransCommerceOrder();
        if ($transactionData = $objMidtransOrder->getTransactionDetailsByMidtransTransaction(
            $transaction_id
        )) {
            $cartID = $transactionData['id_cart'];
            // if orders found for the transaction then set the order status
            if ($orders = WkMidtransCommerceHelper::getOrdersByCartId($cartID)) {
    ...
    public function orderCompleted($eventData)
    {
        ...
        foreach ($purchaseUnits as $purchase) {
            ...
            if ($transactionData = $objMidtransOrder->getTransactionDetailsByMidtransTransaction(
                ...
                // Check if Midtrans order is already finished with status COMPLETED
                ...
                $this->updateOrderMidtransOrderStatus($transaction_id, $payment_status, $eventData['resource']);
            }
        }
    }
    ...
    public function updateOrderCaptureMidtransOrderStatus($transaction_id, $payment_status)
    {
    ...
    public function updateOrderMidtransOrderStatus($transaction_id, $payment_status, $orderData)
    {
        return Db::getInstance()->execute(
            'UPDATE `'._DB_PREFIX_.'wk_midtrans_commerce_order`
            SET `pp_payment_status` = "'.pSQL($payment_status).'",
