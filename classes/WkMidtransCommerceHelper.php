...
class WkMidtransCommerceHelper
{
    ...
    public static function getAccessToken()
    {
        ...
        if (Tools::isSubmit('submit_midtrans_commerce')) {
            ...
            $wkEnvironment = Tools::getValue('WK_MIDTRANS_COMMERCE_PAYMENT_MODE');
        } else {
        ...
        if ($err) {
            ...
            if (isset($accessToken['error_messages']) && !empty($accessToken['error_messages'])) {
                $apiResp['success'] = false;
                $apiResp['message'] = $accessToken['error_messages'][0];
            } else {
                $apiResp['success'] = true;
                $apiResp['access_token'] = $accessToken['token'];
            }
        }
    ...
    public static function midtransAuthAssertion()
    {
        $temp = array(
            "alg" => "none"
        );
        ...
        $temp = array(
            "iss" => trim(Configuration::get('WK_MIDTRANS_COMMERCE_CLIENT_ID')),
            "payer_id" => Configuration::get('WK_MIDTRANS_COMMERCE_MERCHANT_ID')
        );
        $returnData .= base64_encode(Tools::jsonEncode($temp)) . '.';
        return $returnData;
    }
    ...
    public static function createWebhookUrl($token)
    {
        ...
        if ($token) {
            ...
            if ($wkEnvironment == 'sandbox') {
                $base_url = MidtransHelper::WK_MIDTRANS_SANDBOX_URL;
                $webhookUrl = Context::getContext()->link->getModuleLink('qlomidtranscommerce', 'webhook', array(), true);
            } else {
                $base_url = MidtransHelper::WK_MIDTRANS_LIVE_URL;
                $webhookUrl = Context::getContext()->link->getModuleLink('qlomidtranscommerce', 'callback', array(), true);
            }
    ...
    public static function deleteWebhookUrl()
    {
        ...
        if ($accessToken['success']) {
            ...
            if ($wkEnvironment == 'sandbox') {
                $base_url = MidtransHelper::WK_MIDTRANS_SANDBOX_URL;
                $webhookId = Configuration::get('WK_MIDTRANS_COMMERCE_SANDBOX_WEBHOOK_ID');
            } else {
                $base_url = MidtransHelper::WK_MIDTRANS_LIVE_URL;
                $webhookId = Configuration::get('WK_MIDTRANS_COMMERCE_LIVE_WEBHOOK_ID');
            }
            ...
            curl_setopt_array($curl, array(
                ...
                CURLOPT_HTTPHEADER => array(
                    "Midtrans-Partner-Attribution-Id: " . MidtransHelper::WK_MIDTRANS_COMMERCE_ATTRIBUTION_ID,
    ...
    public function getMidtransOrderItemDetails($idCart)
    {
        $items = array();
    ...
    // returns all the currencies supported by midtrans
    public static function checkMidtransCurrencySuuport($idCart)
    {
        $objCart = new Cart($idCart);
