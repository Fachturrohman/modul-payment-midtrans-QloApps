...
class WkMidtransCommerceRefund extends ObjectModel
{
    // Write all table fields here
    public $order_trans_id;
    public $midtrans_refund_id;
    ...
    // refund types
    const WK_MIDTRANS_COMMERCE_REFUND_TYPE_FULL = 1;
    const WK_MIDTRANS_COMMERCE_REFUND_TYPE_PARTIAL = 2;

    public static $definition = array(
        'table' =>  'wk_midtrans_commerce_refund',
        'primary'   =>  'id_midtrans_commerce_refund',
        'multilang' =>  false,
        'fields'    =>  array(
    ...
    public static function getRefundListByTransID($idTrans)
    {
        $refunds = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            FROM `'._DB_PREFIX_.'wk_midtrans_commerce_refund` pcr
            LEFT JOIN `'._DB_PREFIX_.'wk_midtrans_commerce_order` pco
            ON pcr.`order_trans_id` = pco.`id_midtrans_commerce_order`
            WHERE pcr.`order_trans_id` = ' .(int) $idTrans.'
            ORDER BY pcr.`id_midtrans_commerce_refund` DESC'
        );
